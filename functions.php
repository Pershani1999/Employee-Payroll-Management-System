...
function GetEmployeeSalaryByEmpCodeAndMonth($EmpCode, $month) {
  global $db;
	...
	$salaryData = array();
	$query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "salary` WHERE `emp_code` = '$EmpCode' AND `pay_month` = '$month'");
	if ( $query ) {
		if ( mysqli_num_rows($query) > 0 ) {
...
function GetEmployeeLWPDataByEmpCodeAndMonth($EmpCode, $month) {
    ...
    $loanData['workingDays'] = cal_days_in_month(CAL_GREGORIAN, date('m', strtotime($month)), date('Y', strtotime($month))) - $TotalSundaysAndSaturdays;

    // Total without loans in the payment month
    $query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "loans` WHERE `emp_code` = '$EmpCode' AND `loan_type` = 'loan Without Pay' AND `loan_status` = 'approve'");
    if ( $query ) {
        if ( mysqli_num_rows($query) > 0 ) {
            while ( $row = mysqli_fetch_assoc($query) ) {
                if ( strpos($row['loan_dates'], ',') !== false ) {
                    $loanDates = explode(',', $row['loan_dates']);
                    foreach ( $loanDates as $date ) {
                        $loanDate = date('F, Y', strtotime($date));
                        if ( $loanDate == $month ) {
                            $withoutPay[] = 1;
                        }
                    }
    ...
    $loanData['withoutPay'] = isset($withoutPay) ? array_sum($withoutPay) : 0;
    //====================

    // Total with pay loans till date
    ...
    $query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "loans` WHERE `emp_code` = '$EmpCode' AND `loan_type` != 'loan Without Pay'");
    if ( $query ) {
        if ( mysqli_num_rows($query) > 0 ) {
            while ( $row = mysqli_fetch_assoc($query) ) {
                if ( strpos($row['loan_dates'], ',') !== false ) {
                    $loanDates = explode(',', $row['loan_dates']);
                    foreach ( $loanDates as $date ) {
                        $loanDate = strtotime(date('F, Y', $date));
                        if ( $loanDate <= strtotime($month) ) {
                            $loans[] = 1;
                        }
                    }
                }
    ...
    $loanData['payloans'] = isset($loans) ? array_sum($loans) : 0;
    //====================

    // Total loans in a financial year
    $query = mysqli_query($db, "SELECT * FROM `" . DB_PREFIX . "designations` WHERE `designation` = 'compulsory' AND STR_TO_DATE(`designation_date`, '%m/%d/%Y') BETWEEN '" . $startYear . '-' . $startMonth . '-' . $startDay . "' AND '" . $endYear . '-' . $endMonth . '-' . $endDay . "'");
    if ( $query ) {
        if ( mysqli_num_rows($query) > 0 ) {
            $loanData['totalloans'] = mysqli_num_rows($query) + 7 + 12 + 12 + 6;
        }
    }
